...
['+', '-'],
['*', '/', '%'],
['~', '!', '+', '-', RUNTIME_LITERAL.typeof, RUNTIME_LITERAL.instanceof, RUNTIME_LITERAL.void, RUNTIME_LITERAL.delete],
['++', '--']];
...
export function getObjectExpressionCode(ast, config) {
  ...
  : ''
    ...
    return `${kindAstCode}${getAstCode(key, letInObjectLeftKey(config))}${splitCode}${
      valueAstCode
    }`
...
export function getUnaryExpressionCode(ast, config) {
  ...
  return `${prefix ? operator : ''}${space(config)}${getAstCode(argument, config)}${!prefix ? operator : ''}`;
}
...
export function getCallFunctionExpressionCode(ast, config) {
  return getAstCode(ast._fnAst[RuntimeValue.symbolAst], config)
}
